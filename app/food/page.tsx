...
import data from "@/content/food.json";
...
type FoodItem = { title: string; category: string };
type FoodData = { title?: string; items: FoodItem[] };

export default function FoodPage() {
  const ref = useRef<HTMLDivElement | null>(null);
  const items = (data as FoodData).items;
  useLayoutEffect(() => {
        ...
        <section ref={ref} className="pt-28 md:pt-32 pb-16 px-6 md:px-12 scroll-mt-32">
          <div className="mb-16 flex flex-col md:flex-row items-start md:items-end justify-between border-b border-[#E5E2D8] pb-6 gap-3">
            <div className="max-w-full">
              <span className="text-[10px] font-mono text-[#8F877B] uppercase tracking-widest block mb-2">FOOD</span>
              <h1 className="text-5xl text-[#1C1917] font-light tracking-tight">{(data as FoodData).title ?? "Food Systems Projects"}</h1>
            </div>
            <a href="https://360ace.food" target="_blank" rel="noopener noreferrer" className="interactive group flex items-center gap-3 text-[#1C1917] hover:text-[#8F877B] transition-colors text-xs font-semibold uppercase tracking-widest">
